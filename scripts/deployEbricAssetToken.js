...
// async main
async function main() {
  const EbricAssetTokenContractFactory = await ethers.getContractFactory("EbricAssetToken");
  console.log("Deploying contract...");
  const ebricAssetTokenContract = await EbricAssetTokenContractFactory.deploy();
  await ebricAssetTokenContract.deployed();
  // what's the private key?
  // what's the rpc url?
  console.log(`Deployed contract to: ${ebricAssetTokenContract.address}`);
  // what happens when we deploy to our hardhat network

  if(network.config.chainId === 80001 && process.env.POLYGONSCAN_API_KEY){
    console.log("Waiting for block txes...");
    await ebricAssetTokenContract.deployTransaction.wait(20);
    await verify(ebricAssetTokenContract.address, []);
  }
